...
function resolveRecordPath(body) {
  const ref = body.match(/evidence\/[^\s)`'"]+\/self-review\.md/i);
  if (!ref) return null;
  const candidate = path.normalize(ref[0]).replace(/\\/g, '/');
  return candidate.startsWith('evidence/') ? candidate : null;
}
 ...
 */
function evaluate({ body, prNumber, exists, readFile }) {
  const failures = [];
  const warnings = [];

  const referenced = /evidence\/[^\s)`'"]+\/self-review\.md/i.test(body);
  const recordPath = resolveRecordPath(body);

  if (!referenced || !recordPath) {
    failures.push(
      'No approving review from another identity, and no self-review record linked. ' +
      'In solo mode the self-review ARTIFACT is the review evidence (POAM-008, ' +
      'docs/13-solo-operation.md). Run `/self-review ' + prNumber + '` and link ' +
      '`evidence/<issue>/g3/self-review.md` in the PR body. ' +
      'A heading alone is not a record (#44).');
...
module.exports = { evaluate, sectionFilled, resolveRecordPath, sectionBody };
